...
class OrderEnv3(gym.Env):
    def __init__(
        ...
        # print("env: {}".format(num_cars))
        # print("env: {}".format(num_couriers))
        ...
        dataset_index = args[0].pop("dataset_index", 0)
        ...
        self.num_actions = self.num_cars + self.num_couriers
        ...
        self.car_assigned_num = [0 for _ in range(self.num_cars)]
        self.courier_dispatched_num = 0
        self.car_dispatched_num = [0 for _ in range(self.num_cars)]
        ...
        ORDER_SPACE = spaces.Tuple(
            [
                spaces.Box(low=-np.inf, high=np.inf, shape=(29,)), # order feature
                spaces.Box(low=-np.inf, high=np.inf, shape=(7,)), # pred time
                spaces.Box(low=-np.inf, high=np.inf, shape=()), # entropy
                spaces.Box(low=-np.inf, high=np.inf, shape=()) # uid
            ]
        )

        DISPATCH_MAP = spaces.Box(low=-np.inf, high=np.inf, shape=(7 * self.num_cars + self.num_couriers, 10, 10))
        ...
        OBSERVATION_SPACE_DICT = spaces.Dict(
            {
                ...
                "action_mask": spaces.Box(low=0., high=1., shape=(self.num_actions,))
            }
        )
        ...
        self.observation_space = OBSERVATION_SPACE_DICT if self.wrap_obs else OBSERVATION_SPACE
        self.spec = EnvSpec("OrderEnv-v3")



        data_files = [
            "dataset2train_v3.pkl",
        ...
        self.next_users, self.next_orders, self.next_labels, self.next_dates, self.next_succdelv_td, self.next_is_uav,\
             self.next_is_redelv, self.next_levels, self.next_pred_time = self.sample_day()
        ...
        self.dispatch_state = []
        dispatch_map = [np.zeros([7, 10, 10])] * self.num_cars + [np.zeros([1, 10, 10])] * self.num_couriers
        self.dispatch_map = np.concatenate(dispatch_map)
        saved_actions = {}
        saved_actions.update({
            "car_{}".format(i): [[] for _ in range(7)] for i in range(self.num_cars)
        })
        saved_actions.update({
            "courier": []
        })
        ...
        self.num_lv4 = 0
        self.num_lv3 = 0
        self.disp_order_level_car = [[] for _ in range(self.num_cars)]
    ...
    def get_observation(self):
        if self.steps >= self.spec.max_episode_steps:
            ...
            if self.wrap_obs:
                return {"obs":((np.zeros([29]), np.zeros([7]), 0, 0), self.dispatch_map),
                        "action_mask": np.zeros([self.num_cars])}
            else:
                return (np.zeros([29]), np.zeros([7]), 0, 0), self.dispatch_map
            ...
            pred_time = self.next_pred_time[self.steps]
            pred_time = np.eye(7)[pred_time]
            entropy = self.next_levels[self.steps][1]
            ...
            if self.wrap_obs:
                return {"obs":((order_to_dispatch, pred_time, entropy, uid), self.dispatch_map / 20),
                        "action_mask": np.ones([self.num_actions])}
            else:
                return (order_to_dispatch, pred_time, entropy, uid), self.dispatch_map / 20


    def process_state(self, dispatch_state):
        current_len = len(dispatch_state)
        if self.spec.max_episode_steps > current_len:
            padded_dispatch_state = [i for i in dispatch_state] + [(0, np.zeros([29 + self.num_actions]))
    ...
    def step(self, action):
        ...
        is_car = action < self.num_cars
        if is_car:
            car_id = action
            t = self.next_pred_time[self.steps]
    ...
    def dispatch_orders(self):
        ...
        coords = {
            "car_{}".format(i): (2, 6) for i in range(self.num_cars)
        }
        # coords["courier"] = (2, 6)
        remained_areas = {
            "car_{}".format(i): [[] for _ in range(7)] for i in range(self.num_cars)
    ...
    def compute_reward(self):  # 全1reward，需要entropy来放松假设，解决稀疏反馈大空间搜索问题
        ...
        min_car_order = self.spec.max_episode_steps / 2 / self.num_cars
    ...
    def sample_day(self, episode=None):
        ...
        if episode is not None:
            ...
            day = np.random.choice(len(self.data[self.dataset_index]))
        
        self.episode = day
        day_orders = self.data[self.dataset_index][day]
        ...
        next_users, next_orders, next_labels, next_dates, next_succdelv_td, next_is_uav, next_is_redelv, next_levels, next_pred_time = zip(
            *day_orders)

        return next_users, next_orders, next_labels, next_dates, next_succdelv_td, next_is_uav, next_is_redelv, next_levels, next_pred_time
