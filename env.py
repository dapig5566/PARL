...
class OrderEnv2(gym.Env):
    def __init__(
        ...
        self.num_actions = num_actions = num_cars * 7 + num_couriers
        self.num_hist = num_hist = 7
        CURRENT_STATE_SPACE = spaces.Tuple(
    [
        spaces.Box(low=-np.inf, high=np.inf, shape=(max_episode_length, 29 + num_actions)),
        spaces.Box(low=-np.inf, high=np.inf, shape=(max_episode_length,)),
    ]
)
        ...
        REMAIN_ORDERS_SPACE = spaces.Tuple(
            [
                spaces.Box(low=-np.inf, high=np.inf, shape=(max_episode_length, 29)),
                spaces.Box(low=-np.inf, high=np.inf, shape=(max_episode_length,))
            ]
        )

        DISPATCH_MAP = spaces.Box(low=-np.inf, high=np.inf, shape=(num_actions, 10, 10))
        ...
        logger.info("OrderEnv initialized.")
    ...
    def get_observation(self):
        ...
        # if self.steps >= self.max_episode_steps:
            ...
            if self.wrap_obs:
                return {"obs":((np.zeros([29]), np.zeros([self.num_hist, 36]), 0), self.dispatch_map),
                        "action_mask": np.zeros([self.num_actions])}
            else:
                return (np.zeros([29]), np.zeros([self.num_hist, 36]), 0), self.dispatch_map
            ...
            user_hist = [h for h in user_hist
                         ...
                         and np.argmax(h[17:17+10]).item() == np.argmax(order_to_dispatch[17:17+10]).item()][:self.num_hist]
            
            user_hist = np.array(user_hist)

            assert len(user_hist) <= self.num_hist, ValueError(f"user_hist need to be less than {self.num_hist}, got {len(user_hist)}.")
            if len(user_hist) == 0:
                user_hist = np.zeros([self.num_hist, 36])
            elif len(user_hist) < self.num_hist:
                user_hist = np.concatenate([user_hist, np.zeros([self.num_hist - len(user_hist), 36])])
    ...
    def filter_data(self, order_data):
        ...
        if original_hist_length < self.num_hist:
            return False, (lv, ent)
    ...
    def get_order_level(self, order_data):
        ...
        day_hist_orders = day_hist_orders[:self.num_hist]
